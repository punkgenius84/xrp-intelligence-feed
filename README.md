...
Discovery state is stored locally in `state/discovery.json`, separately from the existing `state/seen.json`. It records per-request validators, successful fetch time, watermarks, candidate first/last-seen metadata, and Federal Register per-term pagination progress, with deterministic age/count retention and atomic writes. Corrupt state fails closed and is preserved. Both discovery state files are git-ignored. GitHub Actions restores and saves both files using a branch-scoped immutable cache lineage; successful workflow runs persist the state for later scheduled or manual runs.
