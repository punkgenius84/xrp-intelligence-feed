...
TREASURY_SOURCE_ID = "treasury-press-releases"
TREASURY_METHOD = "treasury_press_releases"
TREASURY_URL = "https://home.treasury.gov/news/press-releases"
TREASURY_HOST = "home.treasury.gov"
TREASURY_MANIFEST_PATH = "/news-data/press-releases/manifest.json"
TREASURY_SEARCH_PATH = "/news-data/press-releases/search/{year}.json"
_RELEASE_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_-]{2,63}$")
_ARTICLE_PATH = re.compile(r"^/news/press-releases/([A-Za-z0-9][A-Za-z0-9_-]{2,63})/?$")
_DATE_FORMATS = ("%m/%d/%Y", "%B %d, %Y", "%b %d, %Y")
_MAX_PAGE_FETCHES = 3
# Three page-level fetch operations, with a hard shared cap for sends made by
# the HTTP client. Retries and redirects consume this same budget.
_MAX_OUTBOUND_HTTP_ATTEMPTS = 9


class _TreasuryShardNotModified(ValueError):
    def __init__(self, request_id: str):
        super().__init__(f"Treasury search shard {request_id} returned 304 without a cached body")
        self.request_id = request_id


def validate_treasury_source(source: object) -> dict[str, Any]:
    required = {"source_id", "name", "authority_tier", "category", "discovery_method",
                "source_url", "enabled", "lookback_days", "max_items"}
    if not isinstance(source, dict) or set(source) != required:
        raise ValueError(f"Treasury source must have exactly {sorted(required)}")
    ...
    if source["source_id"] != TREASURY_SOURCE_ID:
        raise ValueError(f"source_id must be {TREASURY_SOURCE_ID!r}")
    if source["discovery_method"] != TREASURY_METHOD:
        raise ValueError(f"discovery_method must be {TREASURY_METHOD!r}")
    if source["category"] != "government":
        raise ValueError("Treasury category must be 'government'")
    if source["source_url"] != TREASURY_URL:
        raise ValueError(f"source_url must be the fixed official Treasury listing {TREASURY_URL}")
    if type(source["authority_tier"]) is not int or source["authority_tier"] != 1:
        raise ValueError("Treasury authority_tier must be 1")
...
def _article_identity(value: object) -> tuple[str, str] | None:
    ...
    if raw.startswith("//"):
        return None
    try:
        parts = urlsplit(urljoin(TREASURY_URL, raw))
    except ValueError:
        return None
    ...
    if (parts.scheme.lower() != "https" or not parts.hostname
            or parts.hostname.casefold() != TREASURY_HOST or port not in (None, 443)
            or parts.username is not None or parts.password is not None or not match):
        return None
    slug = match.group(1)
    if not _RELEASE_ID.fullmatch(slug):
        return None
    return slug, f"https://{TREASURY_HOST}/news/press-releases/{slug}"
...
class _TreasuryListingParser(HTMLParser):
    """Parse the current Treasury client-paginated Press Releases HTML listing."""
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if (tag == "div" and self._listing_depth is None
                and "data-news-list" in attr
                and attr.get("data-news-category") == "press-releases"):
            self.listing_found = True
            self._listing_depth = depth
            self.manifest_path = attr.get("data-news-manifest")
            try:
                self.page_size = int(attr.get("data-news-page-size", ""))
            except ValueError:
                self.page_size = None
        if (self._listing_depth is not None and self._row is None
                and depth == self._listing_depth + 1 and tag == "div"):
            self._row = {"anchors": [], "date": "", "category": ""}
            self._row_depth = depth
        if self._row is not None:
            if "date-format" in classes:
                self._date_depth = depth
            if "subcategory" in classes:
                self._category_depth = depth
            if "featured-stories__headline" in classes:
            ...
            if tag == "a":
                ...
                self._anchor = {"href": href, "text": "",
                                ...
                                "is_category": self._category_depth is not None}
        if tag not in self._VOID:
            self._stack.append(tag)

    def handle_startendtag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        # HTML treats a trailing slash on non-void tags such as Treasury's data-news-list
        # div as ignored; it does not close the element as XHTML would.
...
def _parse_listing(content: bytes) -> tuple[list[dict[str, Any]], bool, int]:
    try:
        text = content.decode("utf-8-sig")
    except UnicodeDecodeError:
        text = content.decode("windows-1252")
    parser = _TreasuryListingParser()
    parser.feed(text)
    parser.close()
    if not parser.listing_found:
        raise ValueError("Treasury response is missing the Press Releases listing structure")
    if (parser.manifest_path != TREASURY_MANIFEST_PATH
            or type(parser.page_size) is not int or not 1 <= parser.page_size <= 100):
        raise ValueError("Treasury Press Releases listing metadata is invalid")
    if parser._row is not None or parser._listing_depth is not None or parser._stack:
        raise ValueError("Treasury Press Releases HTML ended inside an incomplete row")

    rows: list[dict[str, Any]] = []
    complete = True
    for raw in parser.rows:
        category_link = any(
            anchor.get("is_category")
            and " ".join(anchor.get("text", "").split()).casefold() == "statements & remarks"
            for anchor in raw["anchors"]
        )
        category = " ".join(raw["category"].split())
        if category_link or category.casefold() == "statements & remarks":
            continue
    ...
    return rows, complete, parser.page_size


def _parse_page(content: bytes) -> tuple[list[dict[str, Any]], bool]:
    rows, complete, _ = _parse_listing(content)
    return rows, complete


def _parse_search_item(item: object) -> dict[str, Any] | None:
    if not isinstance(item, dict):
        raise ValueError("Treasury search item is not an object")
    subcategory = item.get("subcategory", {})
    if not isinstance(subcategory, dict):
        raise ValueError("Treasury search item subcategory is malformed")
    category = " ".join(str(subcategory.get("title", "")).split())
    if (category.casefold() == "statements & remarks"
            or subcategory.get("slug") == "statements-remarks"):
        return None
    identity = _article_identity(item.get("url"))
    title = " ".join(item.get("title", "").split()) if isinstance(item.get("title"), str) else ""
    try:
        published = _parse_date(item.get("date"))
    except ValueError:
        published = None
    if identity is None or not title or published is None:
        raise ValueError("Treasury search item has invalid title, date, or article route")
    return {"native_id": identity[0], "url": identity[1], "title": title,
            "date": published, "category": category or "Press Release"}


class TreasuryPressReleasesDiscovery:
    source_id = TREASURY_SOURCE_ID
    discovery_method = TREASURY_METHOD

    def __init__(self, source: dict[str, Any], *, http: BoundedHttpClient | None = None,
                 now: Callable[[], datetime] = lambda: datetime.now(timezone.utc)):
        self.source = validate_treasury_source(source)
    ...
    def _page_url(self, page: int) -> str:
        if page == 0:
            return TREASURY_URL
        # Treasury's browser pager is one-based; logical page 0 is the unqueried listing.
        return TREASURY_URL + "/?" + urlencode({"page": page + 1})
    ...
    def _candidate(self, row: dict[str, Any], listing_url: str, fetched_at: datetime
        ...
        return DiscoveryCandidate(
            ...
            summary=f"U.S. Treasury Press Release — {category}",
    ...
    def _saved_progress(state: dict[str, Any] | None) -> tuple[dict[str, Any], bool]:
        ...
        if not isinstance(state, dict):
            raise ValueError("Invalid discovery state; refusing to reset the Treasury frontier")
        sources = state.get("sources", {})
        if not isinstance(sources, dict):
            raise ValueError("Invalid discovery sources state; refusing to reset the Treasury frontier")
        source_state = sources.get(TREASURY_SOURCE_ID, {})
        if not isinstance(source_state, dict):
            raise ValueError("Invalid Treasury source state; refusing to reset the frontier")
        pagination = source_state.get("pagination", {})
        if not isinstance(pagination, dict):
            raise ValueError("Invalid Treasury pagination container; refusing to reset the frontier")
        exists = "treasury_press_releases" in pagination
        saved = pagination.get("treasury_press_releases", {})
        if not isinstance(saved, dict):
            raise ValueError("Invalid Treasury pagination state; refusing to reset the frontier")
        required = {"boundary_id", "boundary_date", "deep_page_hint", "frontier_status"}
        if exists and (set(saved) != required or type(saved.get("deep_page_hint")) is not int
            ...
            raise ValueError("Invalid Treasury pagination fields; refusing to reset the frontier")
        boundary_id = saved.get("boundary_id")
        boundary_date = saved.get("boundary_date")
        if exists:
            if boundary_id is None:
                if boundary_date is not None:
                    raise ValueError("Incomplete Treasury boundary state; refusing to reset the frontier")
            elif not isinstance(boundary_id, str) or not _RELEASE_ID.fullmatch(boundary_id):
                raise ValueError("Invalid Treasury boundary_id; refusing to reset the frontier")
            if boundary_date is not None:
                try:
                    parsed_date = date.fromisoformat(boundary_date)
                except (TypeError, ValueError) as exc:
                    raise ValueError("Invalid Treasury boundary_date; refusing to reset the frontier") from exc
                if not isinstance(boundary_date, str) or parsed_date.isoformat() != boundary_date:
                    raise ValueError("Invalid Treasury boundary_date; refusing to reset the frontier")
            ...
            if boundary_id is None and (saved["deep_page_hint"] != 1
                ...
                raise ValueError("Inconsistent empty Treasury frontier; refusing to reset the boundary")
        return saved, exists
    ...
    def collect(self, state: dict[str, Any] | None = None) -> DiscoveryResult:
        ...
        staged_validator_updates: dict[str, dict[str, str]] = {}
        errors: list[str] = []
        page_fetches = 0
        http_calls = 0
        attempt_budget = HttpAttemptBudget(_MAX_OUTBOUND_HTTP_ATTEMPTS)
        items_processed = 0
        failed = False
        page_size: int | None = None
        search_items: list[object] | None = None

        def load_search_items() -> list[object]:
            nonlocal http_calls
            years = list(range(cutoff.year, today.year + 1))
            if len(years) > 2:
                raise ValueError("Treasury lookback unexpectedly spans more than two yearly shards")
            combined: list[object] = []
            # The official listing is client-paginated: it publishes fixed annual JSON
            # search shards, while the visible page= query only updates browser state.
            # Loading at most the two years that can intersect the configured window
            # reproduces those bounded page slices without fetching article pages.
            for year in reversed(years):
                if http_calls >= _MAX_PAGE_FETCHES:
                    raise ValueError("Treasury HTTP call bound reached before page data loaded")
                request_id = f"search_{year}"
                validators = JsonDiscoveryState.request_validators(state or {}, self.source_id,
                                                                     request_id)
                shard_url = f"https://{TREASURY_HOST}" + TREASURY_SEARCH_PATH.format(year=year)
                http_calls += 1
                response = self.http.get(
                    shard_url, etag=validators.get("etag", ""),
                    last_modified=validators.get("last_modified", ""),
                    expected_content_types=("application/json",),
                    attempt_budget=attempt_budget,
                )
                if response.status_code == 304:
                    # No shard body is cached. Clear the unusable validator so a later
                    # run can make an unconditional request instead of looping on 304.
                    state_updates[request_id] = {}
                    raise _TreasuryShardNotModified(request_id)
                try:
                    payload = json.loads(response.content)
                except (UnicodeError, json.JSONDecodeError) as exc:
                    raise ValueError(f"Treasury {year} search shard contains malformed JSON") from exc
                if (not isinstance(payload, dict) or payload.get("category") != "press-releases"
                        or type(payload.get("count")) is not int
                        or not isinstance(payload.get("items"), list)
                        or payload["count"] != len(payload["items"])):
                    raise ValueError(f"Treasury {year} search shard has an incomplete or invalid shape")
                combined.extend(payload["items"])
                staged_validator_updates[request_id] = {
        ...
        def request_page(page: int) -> tuple[bool, bool]:
            nonlocal page_fetches, http_calls, items_processed, failed, page_size, search_items
            ...
            try:
                ...
                if page == 0:
                    ...
                    if response.status_code == 304:
                        ...
                        return True, True
                    rows, complete, page_size = _parse_listing(response.content)
                    etag = response.headers.get("etag", "")
                    last_modified = response.headers.get("last-modified", "")
                    validator_update = {
                    ...
                    if search_items is None:
                        search_items = load_search_items()
                    if page_size is None:
                        raise ValueError("Treasury page size was not initialized")
                    start = page * page_size
                    selected = search_items[start:start + page_size]
                    rows = []
                    complete = True
                ...
                if len(rows) > remaining_items:
                    ...
                    complete = False
                    errors.append("Treasury max_items limit exceeded; page was truncated")
                ...
                if not complete:
                    errors.append(f"Malformed Treasury listing row on page {page}")
                    failed = True
                    return False, False
                if page == 0:
                    staged_validator_updates[request_id] = validator_update
                return True, False
            except _TreasuryShardNotModified as exc:
        ...
        def commit_staged_validators() -> None:
            state_updates.update(staged_validator_updates)
        ...
        if not_modified:
            ...
            if has_state:
                ...
                pagination["treasury_press_releases"] = progress
        ...
        if not has_state:
            ...
            pagination = {"page_fetches": page_fetches, "treasury_press_releases": progress}
        ...
        if boundary_id is None:
            ...
            return DiscoveryResult(self.source_id, self.discovery_method,
                                   ...
                                   pagination={"page_fetches": page_fetches,
                                               "treasury_press_releases": progress})
